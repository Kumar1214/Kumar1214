...
const ImageUploader = ({ value = [], onChange, multiple = false, loading = false, label }) => {
    ...
    const handleDrop = async (e) => {
        ...
        if (e.dataTransfer.files && e.dataTransfer.files[0]) {
            ...
            const { newUrls, skippedCount } = await processFiles(files);

            if (skippedCount > 0) {
                // toast.error(`Skipped ${skippedCount} non-image file(s)`); // skippedFilesCount was undefined in original code, fixing logical error too if intent was to use it
            }
        ...
        <div>
            ...
            {value.length > 0 && (
                <div style={{ display: 'flex', gap: '12px', marginBottom: '12px', flexWrap: 'wrap' }}>
                    {value.map((url, index) => (
                        <div key={url || `img-${index}`} style={{ position: 'relative', width: '100px', height: '100px' }}>
                            <img
                                src={getImageUrl(url)}
                                alt={`Preview ${index + 1}`}
                                style={{
