import { Plus, Trash2, Edit, CheckSquare, X, BarChart2, TrendingUp, Users, CheckCircle, Share2 } from 'lucide-react';
    ...
    return (
        <div>
            ...
            <FormModal isOpen={showExamModal} onClose={() => setShowExamModal(false)} title={editingExam ? 'Edit Exam' : 'Create New Exam'} onSubmit={handleExamSubmit} isLoading={uploading}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                    <div>
                        <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.9rem', fontWeight: 500 }}>Price ({currencySymbol}) *</label>
                        <input
                            type="number"
                            min="0"
                            required
                    ...
                    <div style={{ maxHeight: '200px', overflowY: 'auto', border: '1px solid #D1D5DB', borderRadius: '4px', padding: '8px', backgroundColor: '#F9FAFB' }}>
                        {newExam.prizes && parseArrayField(newExam.prizes).length > 0 ? (
                            parseArrayField(newExam.prizes).map((prize, idx) => (
                                <div key={prize.id || `prize-${prize.title || 'new'}-${idx}`} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 3fr auto', gap: '8px', marginBottom: '8px', padding: '8px', background: 'white', borderRadius: '4px' }}>
                                    <input
                                        type="text"
                    ...
                    <div style={{ maxHeight: '200px', overflowY: 'auto', border: '1px solid #D1D5DB', borderRadius: '4px', padding: '8px', backgroundColor: '#F9FAFB' }}>
                        {newExam.winners && parseArrayField(newExam.winners).length > 0 ? (
                            parseArrayField(newExam.winners).map((winner, idx) => (
                                <div key={winner.id || `winner-${winner.rank || idx}-${winner.name || 'new'}`} style={{ display: 'grid', gridTemplateColumns: '60px 2fr 1fr 1fr 2fr auto', gap: '8px', marginBottom: '8px', padding: '8px', background: 'white', borderRadius: '4px' }}>
                                    <input
                                        type="number"
                                        placeholder="Rank"
                                        value={winner.rank}
                    ...
                    <div style={{ maxHeight: '200px', overflowY: 'auto', border: '1px solid #D1D5DB', borderRadius: '4px', padding: '8px', backgroundColor: '#F9FAFB' }}>
                        {newExam.studyMaterial && parseArrayField(newExam.studyMaterial).length > 0 ? (
                            parseArrayField(newExam.studyMaterial).map((mat, idx) => (
                                <div key={mat.id || `material-${mat.title || 'new'}-${idx}`} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 3fr auto', gap: '8px', marginBottom: '8px', padding: '8px', background: 'white', borderRadius: '4px' }}>
                                    <input
                                        type="text"
                                        placeholder="Title"
                                        value={mat.title}
                            ...
                            <div style={{ flex: 1, overflowY: 'auto', border: '1px solid #E5E7EB', borderRadius: '4px' }}>
                                {questions.map(q => (
                                    <button
                                        ...
                                        style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', border: 'none', borderBottom: '1px solid #F3F4F6', cursor: 'pointer', backgroundColor: (newExam.questions || []).includes(q.id) ? '#EFF6FF' : 'white', textAlign: 'left' }}
                                    >
                                ...
                                {questionFormData.type === 'multiple-choice' && (
                                    <div>
                                        ...
                                        {questionFormData.options.map((option, index) => (
                                            <div key={`option-${index}`} style={{ display: 'flex', gap: '8px', marginBottom: '8px', alignItems: 'center' }}>
                                                <input
                                                    type="radio"
                                                    name="correctAnswer"
