...
import { entertainmentService } from "../../../services/api";
...
export default function MusicCategories() {
  ...
  const fetchCategories = async () => {
    try {
      const response = await entertainmentService.getMusicGenres();
  ...
  const saveCategory = async () => {
    try {
      if (editData.id) {
        await entertainmentService.updateMusicGenre(editData.id, editData);
        toast.success("Category updated");
      } else {
        await entertainmentService.createMusicGenre(editData);
  ...
  const handleDelete = async () => {
    try {
      await entertainmentService.deleteMusicGenre(deleteId);
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow">
      <div className="flex justify-between mb-4">
        <h2 className="text-xl font-semibold">Music Categories</h2>
      ...
      <table className="w-full border-collapse">
        ...
        <tbody>
          {loading ? (
            ...
            <tr><td colSpan="4" className="p-3 text-center">No categories found</td></tr>
          ) : (
            categories
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-xl w-72 shadow-xl">
