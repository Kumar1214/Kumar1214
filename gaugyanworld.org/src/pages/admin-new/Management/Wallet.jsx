...
import { Wallet as WalletIcon, Plus, Minus, Search, TrendingUp, X, Coins, Settings, Filter } from 'lucide-react';
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-md border border-orange-100 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div className="relative">
                                {/* Ideally this should follow currency too, but for input suffix/prefix we might just use text or get symbol */}
                                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">{formatPrice(0).replace(/\d|\./g, '').trim()}</span>
                                <input
                                    type="number"
                                    value={coinRate}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-2xl font-bold text-gray-900">{formatPrice(totalBalance)}</p>
                        </div>
                    </div>
                    <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 flex items-center gap-4">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {filteredUsers.map(user => (
                                    <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 font-bold text-gray-900">
                                            {user.earnings > 0 ? (
                                                <span className="text-green-600">{formatPrice(user.earnings)}</span>
                                            ) : (
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col gap-2">
                                                <div className="flex items-center justify-center gap-2">
                                                    ...
                                                    >
                                                        <Plus size={14} /> {formatPrice(0).replace(/\d|\./g, '').trim()}
                                                    </button>
                                                    <button
                                                    ...
                                                    >
                                                        <Minus size={14} /> {formatPrice(0).replace(/\d|\./g, '').trim()}
                                                    </button>
                                                </div>
                                                <div className="flex items-center justify-center gap-2">
                                                    <button
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                    {modalMode.includes('coin') ? 'Coins (Count)' : `Amount (${formatPrice(0).replace(/\d|\./g, '').trim()})`}
                                </label>
                                <input
                                    type="number"
                                    min="1"
