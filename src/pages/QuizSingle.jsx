...
import { Clock, HelpCircle, Trophy, Users, ArrowLeft, Gift, Award, Calendar, Share2, Bookmark, FileText } from 'lucide-react';
...
const QuizSingle = () => {
    ...
    // Mock Date logic fallback if dates are missing, or parsed from API
    const startDate = quiz.startDate ? new Date(quiz.startDate) : new Date();
    const endDate = quiz.endDate ? new Date(quiz.endDate) : new Date();
                ...
                <div>
                    ...
                    {!isEnded ? (
                        <Button fullWidth onClick={() => navigate(`/quiz/${id}/start`)}>
                            {quiz.price === 0 ? 'Start Quiz Now' : `Pay ₹${quiz.price} to Enter`}
                        </Button>
                    ) : (
                    ...
                    <div style={{ marginTop: 'var(--spacing-xl)' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                            {quiz.prizes.map((prize, idx) => (
                                <div key={idx} className="prize-card" style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', backgroundColor: '#FEF3C7', borderRadius: '8px', border: '1px solid #FCD34D', transition: 'transform 0.2s' }}>
                ...
                <div>
                    ...
                    <div style={{ marginBottom: 'var(--spacing-xl)' }}>
                        ...
                        <ul style={{ lineHeight: 2, color: 'var(--color-text-muted)', paddingLeft: '20px' }}>
                            {quiz.topics.map((topic, idx) => (<li key={idx}>{topic}</li>))}
                        </ul>
                    </div>
