const Product = require('../../modules/marketplace/Product');
const Order = require('../../modules/marketplace/Order');
const VendorProfile = require('../../modules/marketplace/VendorProfile');
...
// @desc    Create a new vendor (Admin) - Creates User + Vendor Profile
// @route   POST /api/ecommerce/vendors/admin/create
// @access  Private/Admin
exports.createVendorByAdmin = asyncHandler(async (req, res) => {
  ...
  if (!user) {
    ...
    // If user exists, upgrade role if not admin
    if (user.role !== 'admin') {
      user.role = 'vendor';
      await user.save();
    }
  ...
  const vendorExists = await VendorProfile.findOne({ where: { userId: user.id } });
  if (vendorExists) {
    res.status(400);
    throw new Error('Vendor profile already exists for this user');
  }
  ...
  const vendor = await VendorProfile.create({
    ...
    storeDescription: `Store for ${businessName}`, // Default description
    address,
    contactEmail: email,
    contactPhone: phone,
    gstNumber: null, // Optional in this flow
    status: 'active' // Auto-approve if created by admin
  });
